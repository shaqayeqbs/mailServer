...
import { addEvents } from "../../@core/api/events";
...
function AddNewEvents() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      !formData.eventRelation ||
      !description
    ) {
      return toast.error("Please fill in all required fields");
    }
    console.log(formData.age, formData.gender, formData.name);
    if (showhelp) {
      if (!formData.name || !formData.age || !formData.gender) {
        return toast.error("اطلاعت مورد نیاز برای کمک را وارد کنید");
      }
    }
    ...
    const dataToSend = {
      ...
      Date: formattedDate,
      Relation_with_you: formData.eventRelation,
    ...
    try {
      const res = await addEvents(dataToSend);
      console.log(res);
      if (res?.data.status === "ok") {
        addEvent(res.data); // Assuming res.data contains the added event
      ...
      <form onSubmit={handleSubmit} className="max-w-md  mx-auto">
        ...
        <div className="mb-4">
          ...
          <Select
            ...
            onChange={(selectedOption) =>
              handleInputChange("eventRelation", selectedOption.value)
            }

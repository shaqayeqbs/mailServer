...
export const getShopBestSales = async ({ page }) => {
  try {
    const res = await instance.get(`${SHOP_END_POINTS.shop_best_sells}${page}`);
    return res?.data?.data;
  } catch (err) {
    return handleRequestError(err);
  }
};
export const getShopGiftPack = async ({ page }) => {
  try {
    const res = await instance.get(`${SHOP_END_POINTS.shop_gift_pack}${page}`);
...
export const getShopEconomial = async ({ page }) => {
  try {
    const res = await instance.get(`${SHOP_END_POINTS.shop_economial}${page}`);
